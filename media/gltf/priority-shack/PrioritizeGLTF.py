...
def FixGLTF(gltf, pimg, ind_fix, prim_fix):
    ...
    for i in range(len(prim_fix)):
        ...
        primitive["primitives"].append(prims[prim_fix[i]][0])
...
def FixNodes(ingltf, outgltf, buffer_number):
    if 'nodes' in ingltf:
        outgltf["nodes"] = [copy.deepcopy(ingltf["nodes"][buffer_number])]
...
def FixMeshes(ingltf, outgltf, buffer_number):
    ...
    if "TEXCOORD_0" in outgltf["meshes"][0]["primitives"][0]["attributes"]:
        ...
        index += 1
    if "TEXCOORD_1" in outgltf["meshes"][0]["primitives"][0]["attributes"]:
        outgltf["meshes"][0]["primitives"][0]["attributes"]["TEXCOORD_1"] = index
        index += 1
...
if __name__ == "__main__":
    ...
    Camera = {
    "center": np.array([-5.23,+5.2,-0.82]),
    "orientation": np.quaternion(1.0,0.0,0.0,0.0),
    "ratio": 16.0/9.0,
    "fov": 90.0,
    "near": 0.1,
    "far": 10000.0,
